...
describe('JwtAuthGuard', () => {
  ...
  it('accepts a valid token and loads the current role from the RLS-bound client', async () => {
    ...
    const guard = new JwtAuthGuard({
      getAuthClient: () => ({ auth: { getUser: jest.fn().mockResolvedValue({ data: { user: { id: 'user-1', email: 'beto@example.com' } }, error: null }) } }),
      getUserClient: jest.fn(() => ({ from })),
    } as never);
    ...
    expect((guard as any).supabase.getUserClient).toHaveBeenCalledWith('token');
  });

  it('rejects a blocked account', async () => {
    const guard = new JwtAuthGuard({
      getAuthClient: () => ({ auth: { getUser: jest.fn().mockResolvedValue({ data: { user: { id: 'user-1', email: 'beto@example.com' } }, error: null }) } }),
      getUserClient: jest.fn(() => ({ from: jest.fn(() => ({ select: jest.fn(() => ({ eq: jest.fn(() => ({ maybeSingle: jest.fn().mockResolvedValue({ data: { id: 'user-1', tipo_perfil: 'PASSAGEIRO', status_conta: 'BLOQUEADO' }, error: null }) })) })) })) })),
    } as never);
    await expect(guard.canActivate(context('Bearer token'))).rejects.toBeInstanceOf(UnauthorizedException);
  });
});
